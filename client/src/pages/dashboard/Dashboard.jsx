...
function ProgressChart({ progressList, totalEcoPoints = 0 }) {
  const data = {
    labels: ['Materials Completed', 'Quizzes Completed', 'Assignments Submitted', 'Games Completed'],
    datasets: [
      {
        label: 'Progress',
        data: [
          progressList.reduce((sum, p) => sum + p.materialsCompleted, 0),
          progressList.reduce((sum, p) => sum + p.quizzesCompleted, 0),
          progressList.reduce((sum, p) => sum + p.assignmentsSubmitted, 0),
          progressList.reduce((sum, p) => sum + p.gamesCompleted, 0)
        ],
        backgroundColor: [
          'rgba(54, 162, 235, 0.8)',
          'rgba(75, 192, 192, 0.8)',
  ...
  const ecoPointsData = {
    ...
    datasets: [
      {
        data: [totalEcoPoints, Math.max(0, 50 - totalEcoPoints)],
        backgroundColor: [
          'rgba(75, 192, 192, 0.8)',
          'rgba(255, 99, 132, 0.2)'
        ],
  ...
  return (
    <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
      ...
      <div style={{ width: '300px' }}>
        <h4>EcoPoints Progress (0/50)</h4>
        <Doughnut data={ecoPointsData} />
        <p style={{ textAlign: 'center', marginTop: '10px' }}>
          <strong>{totalEcoPoints}/50 points</strong>
        </p>
      </div>
    </div>
  )
...
function Leaderboard() {
  ...
  useEffect(() => { 
    // Use student-accessible endpoint
      ...
      <ol className="leaderboard-list">
        {topStudents.map((student, index) => (
          <li key={student._id} className="leaderboard-item">
            ...
            <span className="points">{student.ecoPoints || 0} ecoPoints</span>
          </li>
        ))}
      </ol>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Load user's ecoPoints
    api.get('/auth/me').then(r => {
      setTotalEcoPoints(r.data.data.ecoPoints || 0)
    }).catch(() => setTotalEcoPoints(0))
  ...
  async function markCourseCompleted(courseId) {
    try {
      ...
      // Update local ecoPoints state
      setTotalEcoPoints(prev => prev + 10)
        ...
        {showProgress && (
          <div className="modal-overlay" onClick={()=>setShowProgress(false)}>
            <div className="modal" onClick={(e)=>e.stopPropagation()} style={{maxWidth: 800}}>
              ...
              <div className="modal-body">
                <ProgressChart progressList={progressList} totalEcoPoints={totalEcoPoints} />
